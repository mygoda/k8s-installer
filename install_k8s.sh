...
export KUBERNETES_SLAVE_HOSTNAME=$KUBERNETES_CLUSTER_ID-slave1
export KUBERNETES_SLAVE1_HOSTNAME=$KUBERNETES_CLUSTER_ID-slave2
export K8S_ALL_NODE=$KUBERNETES_SLAVE_HOSTNAME,$KUBERNETES_SLAVE1_HOSTNAME
...
update_services_config() {
  ...
  if [[ $INSTALLER_TYPE == 'master' ]]; then
    ...
    echo -e "KUBE_CONTROLLER_MANAGER_OPTS=\"--address=0.0.0.0 --master=127.0.0.1:8080 --machines=$K8S_ALL_NODE --v=0 \"" | sudo tee -a /etc/default/kube-controller-manager
  ...
  elif [[ $INSTALLER_TYPE == 'slave1' ]]; then
    echo '######### Updating configurations for kubernetes slave1 ############'

    # update kubelet config
    echo "KUBELET=$KUBERNETES_EXECUTABLE_LOCATION/kubelet" | sudo tee -a /etc/default/kubelet
    echo "KUBELET_OPTS=\"--address=0.0.0.0 --port=10250 --hostname_override=$KUBERNETES_SLAVE1_HOSTNAME --api_servers=http://$KUBERNETES_MASTER_HOSTNAME:8080 --etcd_servers=http://$KUBERNETES_MASTER_HOSTNAME:4001 --enable_server=true --logtostderr=true --v=0\"" | sudo tee -a /etc/default/kubelet
